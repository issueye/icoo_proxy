<template>
    ...
    >
        <!-- 头像 -->
        <div
            class="flex-shrink-0 w-9 h-9 rounded-md flex items-center justify-center text-xs font-semibold shadow-sm"
            :class="
                isUser
        ...
        >
                ...
                <template v-for="item in traceItems" :key="item.id">
                    ...
                    >
                        <button
                            ...
                            class="flex items-center gap-1.5 text-xs text-muted-foreground hover:text-primary transition-colors px-1"
                        >
                            <BrainIcon :size="14" />
                            <span>{{
                        ...
                        <div
                            ...
                            class="mt-2 p-3 bg-secondary rounded-md border border-border text-xs text-muted-foreground whitespace-pre-wrap max-h-60 overflow-y-auto"
                        >
                            {{ item.content }}
                        </div>
                    </div>
            ...
            <div
                ...
                class="rounded-md px-4 py-3 text-sm leading-relaxed relative shadow-sm"
                :class="
                    isUser
            ...
            >
                ...
                <div v-else>
                    ...
                    >
                        <div class="w-2 h-2 rounded-full bg-primary dot-1"></div>
                        <div class="w-2 h-2 rounded-full bg-primary dot-2"></div>
                        <div class="w-2 h-2 rounded-full bg-primary dot-3"></div>
                    </div>
                </div>
            </div>
            ...
            <div class="flex items-center gap-2 mt-2 px-1">
                ...
                <span v-if="isUser && message.status" class="flex items-center gap-1">
                    <LoaderIcon v-if="message.status === 'sending'" :size="12" class="text-muted-foreground animate-spin" />
            ...
            >
                <button
                    @click="copyContent"
                    class="inline-flex items-center gap-1.5 px-2 py-1 rounded-md hover:bg-secondary text-muted-foreground hover:text-foreground transition-colors text-xs"
                    :title="copied ? '已复制' : '复制'"
                >
