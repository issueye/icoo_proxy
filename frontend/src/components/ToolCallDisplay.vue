<template>
    <div class="tool-call-container">
        <div
            ...
            :key="toolCall.id"
            class="tool-call-item mb-2 bg-secondary rounded-md border border-border overflow-hidden"
        >
            <!-- 工具头部 -->
            <button
                @click="toggleExpand(toolCall.id)"
                class="w-full flex items-center justify-between px-2 text-left hover:bg-secondary transition-colors"
            >
            ...
            >
                ...
                <div class="px-3 py-2 border-b border-border">
                    ...
                    <pre class="text-xs text-muted-foreground bg-background p-2 rounded overflow-x-auto whitespace-pre-wrap break-all">{{ formatArguments(toolCall.arguments) }}</pre>
                </div>
                ...
                <div v-if="toolCall.status !== 'running'" class="px-3 py-2">
                    ...
                    <pre
                        ...
                        :class="toolCall.status === 'error' ? 'text-red-400 bg-red-900/20' : 'text-muted-foreground bg-background'"
