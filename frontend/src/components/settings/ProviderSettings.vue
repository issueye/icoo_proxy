<template>
    <ManagePage
        ...
        :primary-action="{ key: 'add', label: '添加供应商' }"
        @search="handleSearch"
    ...
    >
        <template #cell-name="{ row: provider }">
            <div class="flex items-start gap-3">
                <div :class="[
                    ...
                    <component :is="getProviderIcon(provider.type)" :size="18"
                        :class="provider.enabled ? 'text-green-500' : 'text-muted-foreground'" />
                </div>
                <div>
                    <span class="font-semibold text-foreground">{{ provider.name }}</span>
                    <p v-if="provider.api_base" class="text-[10px] text-muted-foreground mt-0.5 truncate max-w-[200px]">
                        {{ provider.api_base }}
                    </p>
                </div>
            </div>
        ...
        <template #cell-models="{ row: provider }">
            <div class="space-y-1.5">
                <div v-if="provider.default_model" class="flex items-center gap-1 text-xs text-accent">
                    <StarIcon :size="10" />
                    {{ provider.default_model }}
                </div>
                <div v-if="getModelCount(provider)" class="flex items-center gap-1 text-xs text-muted-foreground">
                    <LayersIcon :size="10" />
                    {{ getModelCount(provider) }} 个模型
                </div>
                <div v-if="getModelLabels(provider).length > 0" class="flex flex-wrap gap-1">
                    <span
                        v-for="label in getModelLabels(provider)"
                        :key="label"
                        class="px-2 py-0.5 text-[10px] bg-secondary text-muted-foreground rounded"
                    >
                        {{ label }}
                    </span>
                    <span v-if="getExtraModelCount(provider) > 0" class="px-2 py-0.5 text-[10px] text-muted-foreground">
        ...
        <!-- AI Agent 默认模型设置 -->
        <div class="bg-secondary rounded-md border border-border p-5 flex-shrink-0 mb-3">
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-4">
                    <div class="w-11 h-11 rounded-md bg-accent/15 flex items-center justify-center">
            ...
            <div class="mt-3 flex items-center gap-2">
                ...
                <span v-if="agentDefaultModel" class="inline-flex items-center gap-1 px-2 py-0.5 bg-accent/10 text-accent text-xs font-medium rounded-md">
        ...
        <div v-else-if="filteredProviders.length > 0" class="space-y-3 overflow-y-auto pr-1 flex-1">
            <div
                ...
                :key="provider.id"
                class="bg-secondary rounded-md border border-border p-4 hover:border-accent/30 transition-all group"
            >
                <div class="flex items-start justify-between">
                    <div class="flex items-start gap-4">
                        ...
                        <div class="min-w-0">
                            ...
                            <div class="mt-2 flex items-center gap-3 flex-wrap">
                                ...
                                <span v-if="getModelCount(provider)" class="inline-flex items-center gap-1 text-xs text-muted-foreground">
                                    <LayersIcon :size="10" />
                                    {{ getModelCount(provider) }} 个模型
                                </span>
                                <span v-if="provider.api_base" class="text-[10px] text-muted-foreground truncate max-w-[200px]">
                                    {{ provider.api_base }}
                                </span>
                            </div>
                    ...
                    <div class="flex items-center gap-1 opacity-60 group-hover:opacity-100 transition-opacity">
                        <button
                            @click="openEditProvider(provider)"
                            class="btn btn-ghost btn-icon text-muted-foreground hover:text-accent"
                            title="编辑"
                        >
                            <EditIcon :size="15" />
                        </button>
        ...
        <div
            v-else
            class="bg-secondary rounded-md border border-border p-10 text-center flex-1 flex flex-col items-center justify-center"
        >
            <div class="w-14 h-14 mx-auto mb-4 rounded-md bg-secondary flex items-center justify-center">
                <BotIcon :size="26" class="text-muted-foreground" />
            </div>
            <h3 class="text-sm font-medium text-foreground mb-1">{{ searchQuery || filterStatus ? "没有找到匹配的供应商" : "暂无供应商配置" }}</h3>
            <p class="text-xs text-muted-foreground mb-4">{{ searchQuery || filterStatus ? "试试其他筛选条件" : "添加 AI 模型供应商来开始使用" }}</p>
            <button
        ...
        >
            <div class="space-y-5">
                ...
                <div class="bg-secondary rounded-md p-4 space-y-4">
                    ...
                    <div>
                        <label class="block text-sm text-muted-foreground mb-2">供应商类型</label>
                        ...
                        <p v-if="editingProvider" class="text-[11px] text-muted-foreground mt-2 flex items-center gap-1">
                            <LockIcon :size="10" />
                            编辑模式下供应商类型不可修改
                        </p>
                    </div>
                    ...
                    <div>
                        <label class="block text-sm text-muted-foreground mb-2">供应商名称</label>
                        <Input
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        />
                        <label for="provider-enabled" class="text-sm text-muted-foreground">
                            启用此供应商
                        </label>
                    </div>
                </div>
                ...
                <!-- API 配置 -->
                <div class="bg-secondary rounded-md p-4 space-y-4">
                    <h4 class="text-xs font-semibold text-muted-foreground uppercase tracking-wider">API 配置</h4>

                    <div>
                        <label class="block text-sm text-muted-foreground mb-2">API Key</label>
                        <div class="relative">
                            <Input
                    ...
                    <div v-if="providerForm.type === 'minimax'">
                        <label class="block text-sm text-muted-foreground mb-2">兼容 API 格式</label>
                        <Select
                ...
                <!-- 模型配置 -->
                <div class="bg-secondary rounded-md p-4 space-y-4">
                    <h4 class="text-xs font-semibold text-muted-foreground uppercase tracking-wider">模型配置</h4>
                    ...
                    <div>
                        <div class="flex items-center justify-between mb-2">
                            <label class="text-sm text-muted-foreground">支持的模型</label>
                            <button
                                @click="addModel"
                                type="button"
                        ...
                        <div v-if="providerForm.models.length > 0" class="space-y-2">
                            ...
                            >
                                ...
                                <button
                                    ...
                                    type="button"
                                    class="p-2 rounded-md hover:bg-secondary text-muted-foreground hover:text-red-500 transition-colors"
                                >
                                    <XIcon :size="15" />
                                </button>
                            </div>
                        ...
                        <div
                            v-else
                            class="py-4 text-center border border-dashed border-border rounded-md"
                        >
                            <p class="text-xs text-muted-foreground">暂未添加模型</p>
                            <button
                                @click="addModel"
                                type="button"
        ...
        >
            <div class="space-y-4">
                <div>
                    <label class="block text-sm text-muted-foreground mb-2">模型名称</label>
                    <Input
                ...
                <div v-if="agentModelForm.providers.length > 0">
                    <label class="block text-sm text-muted-foreground mb-2">从供应商模型中选择</label>
                    <div class="space-y-3 max-h-[280px] overflow-y-auto pr-1">
                        <div
                            v-for="provider in agentModelForm.providers"
                            :key="provider.id"
                            class="bg-secondary rounded-md p-3"
                        >
                            <div class="flex items-center gap-2 mb-2">
                                <component :is="getProviderIcon(provider.type)" :size="13" class="text-green-500" />
                                <span class="text-xs font-medium text-muted-foreground">{{ provider.name }}</span>
                                <span class="text-[10px] text-muted-foreground uppercase">{{ provider.type }}</span>
                            </div>
                                ...
                                <button
                                    ...
                                    :class="[
                                        ...
                                        agentModelForm.model === `${provider.name}/${llm.model}` || agentModelForm.model === llm.model
                                            ...
                                            : 'bg-secondary text-muted-foreground hover:bg-bg-hover'
                                    ]"
                                >
                                    {{ llm.alias || llm.model }}
                                </button>
...
import { Button, IconButton, Badge, Input, SearchInput, Select } from "@/components/ui";
...
const metrics = computed(() => [
    ...
    },
    {
        ...
        value: agentDefaultModel.value || "未设置",
        label: "Agent 默认模型",
    },
]);
