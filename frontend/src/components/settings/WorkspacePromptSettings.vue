<template>
  <section class="h-full flex-1 flex flex-col min-h-0 gap-4 overflow-hidden">
    <div class="surface-muted rounded-md border border-border p-4 flex-shrink-0">
        ...
        <button
          ...
          :class="[
            ...
            activePrompt === item.name
              ...
              : 'bg-secondary text-muted-foreground border-border hover:text-foreground hover:border-accent/30',
          ]"
        >
          {{ item.label }}
        </button>
    ...
    <div class="surface-panel border border-border rounded-md p-4 flex-1 min-h-0 flex flex-col gap-4 overflow-hidden">
      <div class="flex items-start justify-between gap-4 max-md:flex-col">
        <div>
          ...
          <p class="text-sm text-muted-foreground mt-1">{{ currentPromptMeta.description }}</p>
        </div>
      ...
      <div class="surface-muted rounded-md border border-border p-4 space-y-4 flex-shrink-0">
        <div>
        ...
        <div class="grid grid-cols-[160px_minmax(0,1fr)] gap-3 max-md:grid-cols-1">
          <Select v-model="aiMode" :options="aiModeOptions" placeholder="选择模式" />
          <Input
        ...
        <div v-if="generatedContent" class="bg-secondary border border-border rounded-md p-4">
          ...
          <pre class="text-xs text-muted-foreground whitespace-pre-wrap break-words font-mono">{{ generatedContent }}</pre>
        </div>
      </div>
