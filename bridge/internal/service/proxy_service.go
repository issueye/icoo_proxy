...
func (s *proxyService) Handle(w http.ResponseWriter, r *http.Request, downstream constants.Protocol) {
	...
	// Process plugins: branch BEFORE ConvertRequest; forward raw downstream body.
	if isPluginProvider(route) {
		s.handlePluginRoute(w, r, requestID, downstream, route, start, requestedModel, body)
		return
	}
