...
func TestProxyServiceChatToResponsesPreservesNonStreamPath(t *testing.T) {
	...
	upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("Content-Type", "application/json")
		_, _ = io.WriteString(w, `{"id":"resp_1","object":"response","status":"completed","model":"target-model","output":[{"type":"message","role":"assistant","content":[{"type":"output_text","text":"hello"}]}],"usage":{"input_tokens":1,"output_tokens":1,"total_tokens":2}}`)
	}))
	defer upstream.Close()
	...
	proxy := NewProxyService(
		...
		allowAuth{},
		fixedRouteResolver{route: domain.Route{
			UpstreamProtocol: constants.ProtocolOpenAIResponses,
			Model:            "target-model",
			Provider: domain.ProviderSnapshot{
				BaseURL: upstream.URL,
	...
	req := httptest.NewRequest(http.MethodPost, "/v1/chat/completions", strings.NewReader(`{"model":"requested-model","stream":false,"messages":[{"role":"user","content":"hi"}]}`))
	req.Header.Set("x-api-key", "proxy-key")
	rec := httptest.NewRecorder()
	proxy.Handle(rec, req, constants.ProtocolOpenAIChat)

	if upstreamStream {
		t.Fatal("upstream request unexpectedly enabled streaming")
	}
	if rec.Code != http.StatusOK {
		t.Fatalf("status = %d, body = %s", rec.Code, rec.Body.String())
	}
	if got := rec.Header().Get("Content-Type"); strings.Contains(got, "text/event-stream") {
		t.Fatalf("downstream response unexpectedly streamed: %q", got)
	}
	if !strings.Contains(rec.Body.String(), `"content":"hello"`) {
		t.Fatalf("unexpected converted response: %s", rec.Body.String())
	}
}
