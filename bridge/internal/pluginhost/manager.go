...
// StartEnabled spawns all config entries with enabled=true.
...
// Config enabled=true is only required for StartEnabled (boot); admin may Start any configured entry.
func (m *Manager) Start(ctx context.Context, id string) error {
	entry, ok := m.cfg.Plugins.Entries[id]
	if !ok {
	...
	if err != nil {
		...
		m.plugins[id] = inst
		m.mu.Unlock()
		return fmt.Errorf("pluginhost: dial %s: %w", id, err)
	}
	...
	hs, err := client.Handshake(dialCtx, token, "icoo_llm_bridge")
	if err != nil {
		_ = client.Close()
		_ = cmd.Process.Kill()
		_, _ = cmd.Process.Wait()
		...
		m.plugins[id] = inst
		m.mu.Unlock()
		return fmt.Errorf("pluginhost: handshake %s: %w", id, err)
	}
