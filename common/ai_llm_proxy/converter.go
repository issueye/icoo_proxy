...
func (c *protocolConverter) ConvertStream(input StreamInput) (StreamResult, error) {
	...
	input.Reader = contextReader{ctx: ctx, reader: input.Reader}
	input.Writer = contextWriter{ctx: ctx, writer: input.Writer}
	if input.Downstream == input.Upstream {
...
func (r contextReader) Read(p []byte) (int, error) {
	...
	// Read into private storage because cancellation may return before a
	// non-closable reader exits. The caller is then free to reuse p safely.
...
func (c *protocolConverter) ExtractUsage(protocol constants.Protocol, body []byte) domain.TokenUsage {
	...
	switch protocol {
	case constants.ProtocolAnthropic, constants.ProtocolOpenAIResponses:
	...
	case constants.ProtocolOpenAIChat:
		result = domain.TokenUsage{
			InputTokens:  intFromJSON(usage["prompt_tokens"]),
			OutputTokens: intFromJSON(usage["completion_tokens"]),
			TotalTokens:  intFromJSON(usage["total_tokens"]),
		}
	default:
		return domain.TokenUsage{}
