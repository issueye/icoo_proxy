...
func TestGetAppVersion(t *testing.T) {
	want := os.Getenv("EXPECT_APP_VERSION")
	if want == "" {
		want = "0.0.0-dev"
	}

	if got := NewApp().GetAppVersion(); got != want {
		t.Fatalf("GetAppVersion() = %q, want %q", got, want)
	}
	if !regexp.MustCompile(`^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?(?:\+[0-9A-Za-z.-]+)?$`).MatchString(want) {
		t.Fatalf("default app version %q is not a semantic version", want)
	}
}
