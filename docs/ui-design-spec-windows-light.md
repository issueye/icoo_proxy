...
- 内容表达: 减少说明性文案，优先标签化、结构化和就近提示
- 容器策略: 减少容器嵌容器，优先单层面板直接承载内容
...
6. 少文案，强结构  
   能靠标题、标签、占位符、状态徽标表达的信息，不再额外写成长说明。

7. 少包裹，强主次  
   一个区域尽量只保留一个主容器，避免卡片里再套卡片、面板里再包面板。
...
- 同级区块垂直间距默认使用 `16px`
- 同一行控件间距默认使用 `8px`
- 弹窗和抽屉的 header、body、footer 统一使用固定 padding，不单独为单页改一套
...
- 单个页面默认不超过 3 层视觉容器
- 优先使用“页面区块 + 内容本体”，少用“区块 > 卡片 > 子卡片 > 内嵌模块”
- 同一屏内只保留 1 个主强调区域
- 当内容已通过留白、标题或分割线区分时，不再额外包一层边框容器
...
- 鉴权面板采用标准表单布局，说明文字压缩到最少
- 删除“面板内再套信息卡”的做法，控制区和鉴权区直接落在一级面板中
...
- 固定存在，不能省略为“只有标题文本漂在内容上方”
- 使用 `--dialog-header-padding`
- 包含标题、可选副标题、关闭按钮
- 标题字号建议 `16px`
- 副标题最多 1 行到 2 行
- Header 底部使用分割线，与 Body 明确分区

Body 规则:

- 使用 `--dialog-body-padding`
- 只承载正文、表单、列表、说明
- 默认滚动区域在 Body，不在整个弹窗外层滚动
- Body 内部区块通过留白和分割线区分，避免再套多层容器

Footer 规则:

- 固定存在，哪怕只有一个按钮也保留 footer 区
- 使用 `--dialog-footer-padding`
- 顶部分割线明确与 Body 分离
- 按钮默认右对齐
- 主按钮放最右，取消和次按钮在左侧顺序展开

尺寸与边界:

- 外层圆角固定 `10px`
- Header / Body / Footer 不单独再做不同圆角
- 标准对话框最大高度建议不超过视口高度的 `80vh`

不要:

- 把标题放进 Body 里
- Footer 跟着正文一起滚动
- Header、Body、Footer 各自再套一层装饰卡片
- 每个弹窗单独发明不同的边距、按钮区高度和标题样式

### 8.8 抽屉

规范:

- 适用于编辑详情、查看日志详情、表单配置等不中断主流程的任务
- 默认从右侧滑出
- 宽度建议 `420px`、`560px`、`720px` 三档
- 标题区、正文区、底部按钮区分层明确
...
- 固定存在
- 使用 `--drawer-header-padding`
- 包含标题、可选副标题、关闭按钮
- Header 底部使用分割线

Body 规则:

- 使用 `--drawer-body-padding`
- 为唯一滚动区域
- 优先采用单列表单或单列详情布局
- 抽屉内部避免“卡片套卡片”，用分组标题和间距表达层次

Footer 规则:

- 固定存在
- 使用 `--drawer-footer-padding`
- 顶部分割线
- 按钮默认右对齐
- 支持主操作、次操作、危险操作三层
...
- 抽屉外壳不依赖大圆角塑造层次
- 抽屉内部如有分组容器，统一使用 `8px` 圆角
- 抽屉头部、正文、底部边距与弹窗保持同体系，不单独定义视觉语言
